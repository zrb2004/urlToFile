import { bitable, IOpenAttachment, FieldType, ITable } from "@lark-base-open/js-sdk";
...
function ifFileUploadAndSetSuccess({ id, fileName, resolve }: { id: string; fileName: string; resolve: (value: unknown) => void }) {
  off();
...
export async function checkFileDownloadAndSetCellValue({ url, table, fieldId }: { url: string; table: ITable; fieldId: string }) {
  ...
  urlUploadId[url]?.tableRecord?.forEach((tableRecordId) => {
    ...
    if (cellFilestatus.length >= recordValueCache[tableRecordId].shouldLoadFile) {
      ...
      if (cellFilestatus.every((s) => s.success)) {
        ...
        setCellTask.push(
          table.getCellValue(fieldId, recordId).then((c: any) => {
            // 修改这里：确保shouldSetCellValue是数组格式，支持多附件
            let shouldSetCellValue = allFiles;

            // 如果单元格已经有值，并且配置为不覆盖，则合并现有值和新值---暂不合并
            // const { cover } = getMoreConfig();
            // if (c && !cover) {
            //   // 如果现有值是数组，则合并数组
            //   if (Array.isArray(c)) {
            //     shouldSetCellValue = [...allFiles];
            //   }
            //   // 如果现有值是单个对象，则转换为数组并合并
            //   else {
            //     shouldSetCellValue = [c, ...allFiles];
            //   }
            // }
